...
export const TaskProvider: React.FC<TaskProviderProps> = ({ children, webhookUrl }) => {
  ...
  const saveTask = useCallback(async (taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt'> | Task) => {
    ...
    if (webhookUrl) {
      console.debug(`Enviando tarefa para webhook (${webhookUrl}): ${taskToSave.title}`);
      if (typeof navigator !== 'undefined' && !navigator.onLine) {
        console.warn('Aviso: navegador offline, tentativa de envio mesmo assim.');
      }

      try {
        const result = await postTaskToSheet(webhookUrl, taskToSave);
        console.debug(`Webhook respondeu (${result.status}): ${result.body}`);
      } catch (error: any) {

        const msg = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
        console.error(`Erro ao enviar tarefa: ${msg}`);
        if (error instanceof Error && error.stack) {
          console.error(error.stack);
        }

        alert('Falha ao enviar a tarefa para a planilha.');
      }
    } else {
      console.debug('Webhook não configurado. Tarefa salva apenas localmente.');
